  ...
  useEffect(() => {
        const fetchProducts = async () => {
          try {
            ...
            const startermenuProducts = allProducts.filter((product) =>
              product.categories.some(cat => cat.slug === 'starter-menu')
            );
            // Debug logs
      ...
      <div className="max-w-7xl mx-auto mt-14">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          {products.map((product) => (
            <div className="menu_items flex bg-white rounded-[2rem] p-2" key={product.id}>
